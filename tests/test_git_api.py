...
@patch("toolkit.git_api.subprocess.run")
def test_get_current_branch(mock_run):
    mock_run.return_value = MagicMock(stdout="feature-branch\n", returncode=0)
    branch = get_current_branch()
    assert branch == "feature-branch"
    mock_run.assert_called_once_with(["git", "rev-parse", "--abbrev-ref", "HEAD"], capture_output=True, text=True, check=True)
...
def test_get_diff_empty(mock_run):
    ...
    assert diff == ""
    mock_run.assert_called_once_with(["git", "diff", "main..feature"], capture_output=True, text=True, check=True)
