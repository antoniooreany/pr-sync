...
@patch("toolkit.cli.get_commits", return_value=["commit 1"])
@patch("toolkit.cli.get_changed_files", return_value=["file1.txt"])
@patch("toolkit.cli.find_open_pr", return_value=None)
@patch("toolkit.cli.create_pr", return_value={"url": "https://example.com/pr/1"})
def test_cli_create_pr(mock_create, mock_find, mock_changed_files, mock_commits, mock_diff, mock_branch, mock_auth):
    assert main() == 0
...
def test_cli_update_pr(mock_update, mock_find, mock_changed_files, mock_commits, mock_diff, mock_branch, mock_auth):
    assert main() == 0
